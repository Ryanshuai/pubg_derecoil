...
它当时成了 **per-session 闸门**——跑一次，`night --impulse-off <n>` 传进来。

⚠ **而 2026-08-09 那个技术整个被否掉了,不只是「per-cell 做不到」**：时刻记不准、两套坐标本来就对应不准、**弹药计数器开火时一梭 42 发只读得出约五次**，根本没有逐发分辨率。见 `MODEL.md` 的已排除表。
