# data/templates/ — **代码要加载的东西**

> ⚠ 这个目录以前叫 `docs/training_data/`，标题也一直是那个名字。**里面没有一张训练数据**，也没有一张游戏美术图（那批 2026-08-05 全删了）；`pubg_assets/` 这个子目录名同样是历史遗留，不是一句出处声明。
...
是错的**（`detector/CLAUDE.md` 逐像素复核过）。所以删掉的是一个**没人读、而且
已知不可尽信**的集合——不是一个可惜的语料。姿势识别的准确率归
`detector/posture_detector.py` 的 docstring 持有，这里不复述。
