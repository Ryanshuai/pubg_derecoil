...
## 采集要拒绝的那几样,以及它们其实是同一样(2026-08-08)

**这一层每一次失败都是同一句话:**

> **记录描述的对象,不是被测量的那个对象。**

它出现过五次,每一次都**不抛异常、印出来的数也全都正常**(前四条同一晚,火力模式那条 2026-08-09):
...
| **火力模式** | `ensure_fire_mode` 早就写好,**采集路径一次都没调过** | mg3 两个自动档差 1.50 倍,而没有一梭记得自己是哪一档 |

**前三条和火力模式是拒绝;开镜是记录加警告,不是拒绝** —— 这一句原来写的是「四条现在都是拒绝」,而那从来没成立过。`ads_frac` 在时间坐标那条路上永远是 `nan`(抓帧器只抓 tracker 的 patch,检测器读的是屏幕中心,不在里面),所以架在它上面的闸**通不过**。活着的是 `Magazine.ads_end` 两个端点:开跑前 `ensure_ads()`,扳机松开时读一次。**出镜的梭照样入库,只打警告**——MODEL.md 的库永不删除。

`collect_timed` 的文件头写了每一条的出处。

### 还有一条:**「架上只有这一把枪」**(当天晚些时候,因为上面那些没拦住)

上面第一行「架上两把 mp5k」当时是靠 `control/stock.py` 拒绝的,而那道闸检查的是**它自己动手之前**的架子——第二把枪是它检查之后自己刷的(经过在 `control/CLAUDE.md`)。所以同一个失败在那几条闸都上线之后又发生了一次,**同样五梭、同样 cv 1.4%、同样什么都不抛**。

现在 `read_config` 自己也拒绝:**架上有第二把枪(哪怕是别的型号)就停**。理由不是「乱」,是**无法回答**——这里读 1 号槽,扳机打手里那把,两把枪时屏幕上没有任何东西能说它们是同一把。加上开火前一句显式 `hold(1)`,拿不到就拒绝。

闸门 `pixi run one-gun`,16 条,`ensure_weapon_in_hand` 和 `read_config` 两条独立的路各锁一遍。

⚠ **聚类不是防线,而且它会选错。** 枪托那次它确实切出来了(`5/10, separation 16.3×` 对闸门 8.0),**但那只因为那一格值两倍**。一个值 5% 的件会直接并进邻居格,把均值挪走而没人看得见。

更糟的是 **5 对 5**:干净的五梭和污染的五梭在同一个文件里时,`separation 20.3×` 切得干干净净,而「取最大簇」在等量分裂时**没有意义**——它留下了污染的那半(拟合出 435.2 而不是 669)。**一个在样本量相等时靠掷硬币决定的判据,不是判据。**
...
### 以及:**背包里的事,一次开背包全做完**(2026-08-08)

上面那张表里的「配件」和「镜子」两行,修好之后是**两个各自开背包的函数**:`read_config()` 和 `read_sight()` 各 `with ac.tab_up()`、各 `ac.loadout()`、各关掉。中间隔着什么?量过——**12 个调用,全是 `print` / `sorted` / `dict.get`**,一个驱动游戏的都没有。

```
装配 ensure_kit   开背包 → 关
读配件 read_config 开背包 → 关     ←┐ 中间只有 dict 比较
读镜子 read_sight  开背包 → 关     ←┘
```

**而代价从来不只是慢。两次 `loadout()` 是两次观测**,配件和镜子隔着一次 Tab 开合读回,然后作为**同一把枪**的描述存进同一个样本——就是根 `CLAUDE.md` 第二条法则,只是两次读之间隔的是一秒而不是一天。

现在是 `read_loadout()` 读一次,`read_config(lo, weapon)` / `read_sight(lo)` 退化成**对同一个 dict 的纯函数**。`--kit` 那条路更省一次:读回就在 `ensure_kit` 自己那个 `tab_up` 里做,不出去。**一格标定 3 次开合 → 1 次**,而「两个读描述同一把枪」变成了构造上成立,不需要闸门,也就不会被忘记。

⚠ **这条规矩当时已经写在 `InventoryControl.tab_up()` 的 docstring 里了**:

> Hold ONE across the whole flow; do not wrap each read in its own.

**写下来了,没有机器管,于是这两个函数就是它的反例。** 同一个形状 `ensure_kit` 里也犯过并修过一次(restock 之后的 `hold()`,`False→True→True→False→True`,49 个块,五次按键办一件事;全库 1836 次真按键里 **975 次(58%)**在「结束状态等于开始状态」的块里)。**两次都是同一句散文的两个反例,而散文拦不住第三次。**

**所以现在有机器了:`pixi run layering` 规则 14**,判据和它的三个假阳性写在 `tools/CLAUDE.md`。一句话:**一个语句块里两次进背包、中间只有纯计算 → 红。**

#### ⚠ 而规则 14 看不见 `harness/adapter.measure`,它一格开合了五次(2026-08-09 修)

规则 14 是**一个文件、一跳**——那个限制是买来的(全函数名传递闭包炸到 711 个名字)。所以它看不见「一个函数调五个**别的模块**的方法,每个各自开合一次背包」,而无人值守那条路正是这个形状:

```
find_gun      开→关        clear_rack   开→关
stock_parts   开           find_gun     →关
apply         开→关        read_loadout 开→关          五个循环
```

**而这次不需要新机制,机制早就写好了没人用**:`Kitter.session()` 的 docstring 一字不差就是这条规矩(引的是你 08-06 那句「只是有个 pre-request 说你要检测东西必须在 tab 状态里」),`find_gun`/`clear_rack`/`apply` 里那四个 `finally` 的关闭全部由 `_session` 把门——**而全仓 `.session()` 的调用方只有 `apply()` 自己**。门开着,没有一个调用方走进去。

现在 `measure()` 外面包一个 `with kit.session():`,**五次 → 一次**。两条边界:

- **不许罩住开火**。`_collect` 故意在 `with` 外面——弹匣是 Tab 关着打的,罩进去就是同一个错误升一层。所以拆出了 `_reach()`,它的存在理由只有这个。
- **`_session` 必须存旧值再恢复,不是置 True 再清 False**。`apply()` 自己会在里面再开一层,而旧写法的内层退出会把 `_session` 交还成 `False`,下一个 `find_gun` 就关掉外层正握着的屏幕——**这个类存在的意义,在它防得最多的那种排布下复发**。

⚠ **这一格仍然会为刷新器关一次 Tab,那是对的**,理由在 `control/CLAUDE.md` 的「`leave` 说的是返回时留成什么样」。**判据不是「Tab 翻转次数」,是「这次翻转买到了什么」。**
...
#### ⚠ 而那条警告,2026-08-09 第一次有人去数它印了多少次

p90 那一格 8 梭里 7 梭打了「这一梭被错的常数缩放了」,然后判 `usable`、把曲线写盘了。看起来像 p90 特有的毛病。**一次纯离线查询把这个解释否掉了:**

```
全库 74 个样本文件,42 个含 ads_end=False
  scar   八格全部 100% False
  m762   bare 9/29
  p90    7/8            ← 跟已经出过货的那些格子同一个状态
```

**所以这不是 p90 的事,是一条全库范围、已存在、没人消费的警告。** 而 `agree` **救不了它**:所有梭如果被**同一个**错 K 缩放,那是个共同的乘性因子,在臂间比较里正好约掉——那道闸对这一类是瞎的。

⚠ **还不知道它是真掉镜还是 `AdsDetector` 在开火时翻车**(那个缺陷记在 `detector/CLAUDE.md`,实测静止 387/387、开火 0.79)。`ads_end` 恰好读在扳机松开那一刻,也就是后坐力累积最多、检测器最没被验过的时刻。**在分清之前,别拿它去丢梭,也别假装它不存在。**
...
## 一轮标定怎么排 —— **`MODEL.md` 把这一节指到这里**

`MODEL.md` 只写模型和常数。**「打几梭、怎么轮转」是实验设计,归这一层。**

### 八格必须**在一个 run 里轮转**,不能一格一个 run

`MODEL.md` 6.1 第 2 条记着一个**还没定位的乘性测量错**(空臂跨组散布 2.29× 于抽样噪声,而补偿臂只有 1.07×)。它的关键性质是**乘性、并且一轮之内不变**——所以同一轮里的八格全乘着同一份,做**比值**(配件因子恰好就是比值)时它**约掉**:不是被平均小,是消失。

⚠ **这条只需要「它是乘性的」,不需要知道它是什么。**

一格一个 run 则相反:每格各乘一份自己的。而配件效应本身是 5–25%,`M.K` 那格两条流水线 7.6% 的分歧就是这么来的。
...
### ⚠ 而那个 n 是**每条臂**的,不是每格的 —— `agree` 是臂之间的比较

`agree_spread` 拿每条臂的 `y_true` 均值两两相比,**每条臂权重相同,不管它是 1 梭还是 6 梭**。所以一条 n=1 的臂在这个数里跟一条 n=6 的臂说话一样响,而它自己的 sem 大三倍。

**结论:排一格的时候要排「每臂几梭」,不是「这格几梭」。**

同一把枪、同一个库、同一天,只换分配方式:

```
夜跑 (True, True, 0.8, True, True)   0.8 臂每 5 梭只分到 1 梭
                                     → 臂 n = 1 / 2 / 6 / 6   → spread 19.7%

--scale-sweep 0.5,1.0 --mags 12      每臂 6 梭
                                     → 1815 vs 1813          → spread 0.11%
```

`--mags` 是**总数**,`--scale-sweep` 逐梭轮转,所以 `--mags 2k --scale-sweep a,b` 就是每臂 k 梭。

⚠ **夜跑那个臂计划适合「把曲线迭代到位」,不适合「证明臂之间一致」**——它把绝大多数梭放在满强度上,因为那条臂才是零位测量(`|y_obs|` 小、精度高)。两个目的不同,别拿一个的产物去判另一个。**要判 `agree`,就打一条平衡的两臂 sweep。**

| | |
|---|---|
| 同一 run 内、插花、每臂 n=8 | sem 1.1% → **3% 的真效应看得见** |
| 同一 run 内、比**值**(配件因子) | 乘性的错精确约掉,只剩抽样噪声 |
| 跨 run,**补偿臂** | 抽样噪声就够了,没有额外罚款 |
| 跨 run,**空臂** | 额外 **2%** 的乘性罚款 |
| 任何单 run 的结论 | **必须复现,否则不算** |

### 采集打**补偿臂**,不是空臂

空臂 `y_true = y_obs`,答案 100% 来自测量(`|y_obs|` ≈ 830 counts);补偿臂几乎全来自那条已知曲线(`|y_obs|` ≈ 36)。**任何乘性的测量错都被除以 23。** 这是一个零位测量:越接近答案越精确,而迭代回路本来就往那儿走。

⚠ **空臂仍然要打**——它是 `MODEL.md` 4.1 那道外部检验的一半。只是别让它当主力。

### ⚠ 还没量的那一个数

**轮转的粒度(每轮八格还是每轮两格)由「换一次配件 vs 打一梭」的耗时比决定,而那个比值没量过。** 给 `ensure_kit` 和 `fire_magazine` 计时就有了。

`pixi run plan-order` 已经把**换件次数**压到了下界(2^3 全因子 13 → 7,暴力枚举 8! 验证过是最优),所以剩下的问题只是「一轮铺多宽」。

---

## K 按**比例**测,不按绝对值 —— 否则两个数不可比(2026-08-09)

给一个新镜子量 K 的时候,`calibrate_k` 的输出**不能直接并排放进 `RECOIL_SIGHT_PROFILES`**:

```
red_dot 存的 1.5413      来自 probe_correlator_bias 的**单对**测量
calibrate_k 给的数        丢重复帧 → 累积相关器每对的偏读 b(delta)
```

`config.py` 红点那一整段就是这件事的记录:同一个镜子,丢重复帧读 1.5171、留着读 1.5520,**差 9.4 sigma,而两者唯一的区别是有多少对帧分摊同一段位移。**

**同口径的两条跑相除,那个乘性偏差约掉** —— 跟配件因子用比值的理由完全一样。p90 的做法:

```
同一条命令两边跑        --weapon X --sight Y --ads --inject-s 1.0,24 trials,7 patches
counts  px/frame   ratio   sem
    50      1.71  1.0170  0.24%
   100      3.05  1.0145  0.16%
   200      5.54  1.0178  0.56%
   300      8.37  1.0371  0.68%     ← 排除,规则见下
K = 1.5413 × 1.0157
```

⚠ **排除 300 档的规则是事先定的,不是「它不合群」。** `b(delta)` 在 3 px/pair 附近**变号**,所以比值只在两边落在同一个 delta 时才约得掉——而 K 是用在**真实一梭**上的,那些帧对是 p25 0.90 / 中位 2.00 / p75 3.78 px。50 和 100 档夹着中位数;300 档 8.4 px/frame,在这个常数永远不会被用到的区间外。**200 档在 p75 之外却仍然同意,那是「这条规则不是在凑答案」的检查。**

⚠ **每边只跑一条 = 没复现。** `config.py` 自己写着两次跑之间 K_true 动过 0.55%,是单跑 sem 的九倍。所以这种数是「够用来开火」,不是「已确立的常数」。**它在补偿臂上撑得住**:那条臂 `|y_obs|` 只有 ~36 counts 对 ~830,K 的误差被除以 ~23。

### 量 K 之前:枪必须在手,而且镜子要读回

`calibrate_k --weapon X --sight Y`。两条都不是形式:

- **空手时 `AdsDetector` 会答「已开镜」** —— 它的判据是准星**缺席**,而空手的角色本来就不画准星。`--ads` 会每一趟都读 True 而一次都没开过镜。`ensure_weapon_in_hand` 用**弹药数**证枪在手,那是在场证据不是缺席证据。
- **刷出来的枪戴的是背包里有什么** —— 「我要了红点」和「它戴着红点」是两句话,而这两句之间的缝正是两格 mp5k 差 2.07 倍的那条缝。`--sight` 装完之后 `read_sight` 读回,不一致就拒。
