...
### 第五条:**「架上只有这一把枪」**(当天晚些时候,因为前四条没拦住)

上面第一行「架上两把 mp5k」当时是靠 `control/stock.py` 拒绝的,而那道闸检查的是**它自己动手之前**的架子——第二把枪是它检查之后自己刷的(经过在 `control/CLAUDE.md`)。所以同一个失败在四条闸都上线之后又发生了一次,**同样五梭、同样 cv 1.4%、同样什么都不抛**。
