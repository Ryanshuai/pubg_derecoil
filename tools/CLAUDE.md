...
### 「零调用方」这张单子上，三分之二是量具的毛病（2026-08-15 实测）

一次全仓扫描报了 13 处零调用方，**逐个查过的 9 处里 6 处是假的**，而三个成因各出一次、都是机械的、都跟「这段代码有没有用」无关：

| 假阳性 | 成因 | 怎么当场否掉 |
|---|---|---|
| `calibration/rpm_store.py` 整模块 | **三个调用方是 git 没跟踪的文件**（`hole_groups` / `hole_manual` / `hole_timeline`，另一个 agent 在写） | `grep` 全部 `.py`，不要只扫 `git ls-files` |
| `capture/__init__.py`、`dl_models/__init__.py`、`press/__init__.py` | **包的 `__init__`**：静态零导入，运行时必然加载 | 带 import 追踪跑一遍闸门，三个当场都在名单里 |
| `detector.weapon:sight_tag` | **只被函数体内的 import 调用**（`fit_time_curve.py:1022`），而边表只收模块级 import | 按名字 grep，别信可达图 |

⚠ **第一条是最贵的那条，因为它反着咬**：扫描器看不见未跟踪文件，所以**未跟踪文件里的调用会让被调用者看起来是死的**。这个仓库多 agent 共写、树上常年挂着几十个未跟踪的 `.py`，所以这不是边角情况，是常态。

⚠ **还有第四条，不机械**：`tab_layout.slot_window` 零调用方，但两个 `SKILL.md` 拿它当步骤、`docs/refactor_plan.md` 点名「**是活的，别删**」。**技能里写的步骤在任何 import 图里都看不见**——这条本文件上面已经记过一次，它又咬了一次。

**所以删之前那三道 grep 是硬的，不是谨慎。** 真的三处（`SpawnerSwitcher` / `get_switcher` / `weapon_attachments.calibration_factor`）全都过了这四关才动。

### 而删掉被豁免的代码时，`EXEMPT` 不会提醒你

`check_params.py` 的 `audit` 对两张表**不对称**，而这个不对称没写在任何地方：

```
DEBT   条目对不上任何东西 → 报错（棘轮，本文件上面记过）
EXEMPT 条目对不上任何东西 → 什么都不说
```

于是删掉一个被豁免的文件之后，那条 EXEMPT 会**静默地留成一张永久赦免**——对一个日后可能重新出现的同名路径。所以**豁免条目必须由删代码的那个人手工划掉**，而且没有任何机器会提这一句。2026-08-15 删 `weapon_switcher` 时就是这个形状，划掉它的不是 `params`，是隔壁 `pixi run names`：**替换它的那句注释因为提到了刚被删掉的文件路径而被拒**。
...
```
那个模块的 .pyc（在它的 __pycache__ 下）   11:16:41.442
control/match.py（还原后）                  11:16:41.613     ← 晚 171 ms
```

⚠ 上面第一行故意不写完整路径：**pyc 是瞬态的**，写成路径之后 `pixi run pointers` 会在任何人清过缓存之后报红——2026-08-09 就红过一次，而清缓存的正是本节要求的那条修法。
