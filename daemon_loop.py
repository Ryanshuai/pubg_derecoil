...
**而 `robot.py` 等的那个线程没有任何东西能从里面让它退出**，所以「等 join 返回」这件
事在真实运行里永远不发生：Ctrl-C 一次都送不到。`wait()` 因此存在——**超时不是为了少
等，是为了周期性地回到解释器**，那里才是挂起信号变成异常的地方。

⚠ 这句话写下来的时候是「只有 f13 能让它退出」，而 f13 那条分支**从来没被轮询过**，
所以它当时就已经是「没有任何东西」了。2026-08-10 关闭键整个删掉之后，这句话终于
描述的是代码而不是它的意图。
...
class DaemonLoop:
    ...
    def wait(self, poll_s=0.2):
        ...
        的 join 会把挂起的 SIGINT 一直压到 join 返回为止，而这个循环**没有东西能
        让它返回**。**超时是唯一让主线程周期性回到解释器的东西**，也就是唯一让那
        个信号变成 KeyboardInterrupt 的东西。
