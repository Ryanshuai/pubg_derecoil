...
**所以判据是:在第 k 发上放一个已知尖峰,看哪一发动。** `tools/probe_impulse_ab.py` 做这件事(A/B 差分,把游戏自己的后坐力减掉)。**那个探针 2026-08-08 随弹桶一起删了**,而它的位置由一个更强的检查接替:`harness/verdict.py` 第 4 项,不同强度曲线打的梭必须给同一个 `y_true`。两者是同一个思路——**用一个拟合安排不出来的信号去验它**。

**第一发和最后一发是这个检查最有力的两个点**——不是因为它们重要,而是因为它们**没有邻居可以借**:第 0 发之前没有东西能把误差推给它,最后一发之后没有东西能接住。它们测的是**回路本身**,不是枪。

这跟第一铁律(模板漂移是静默的)是同一句话的两个方向:**那条说「读出来的名字可能是错的」,这条说「读出来的位置可能是错的」,而两者都不会抛异常。**

### ✅ 2026-08-07 实测:m416 的位置是对的,增益不是

`probe_impulse_ab --weapon m416 --bullets 0,1,2,10,20,30,39 --rounds 2`,每个尖峰 −120 counts:

| 命令在第几发 | 落在第几发 | 量到的深度 | 增益 | 次深的桶低多少 |
|---|---|---|---|---|
| 0 | **0** | −90 | **75%** | 78 |
| 1 | **1** | −119 | **99%** | 94 |
| 2 | **2** | −110 | 92% | 89 |
...
**七发七中,包括第一发,而且次深的桶都低 47–94 counts——没有摊到邻居上。** 所以一对一的约束在这把枪上是有实测支撑的,不是假设。

⚠ **但增益不是 1,而且两头最差(0 发 75%,39 发 63%)。** 回路在最后一发只看得见自己发出去的 63%,于是每轮只修掉真实误差的 0.63×α ——**系统性欠修,而且它永远不会知道**,因为它读到的残差本身就是缩水的。

⚠ **两头是这个检查最有力的点,不是因为它们重要,是因为它们没有邻居可借。** 中段 88–92% 而两端 63–75%,这个形状本身就说明缩水来自「边界处没有东西接住」,而不是某一发特殊。

⚠ **一次异常不算数**:第一轮 `spike@1` 整梭跑出 +122(其余都在 −660…−720),第二轮同一个位置反而是全场最准的 99%。**那一梭是坏的,不是第一发不可观测。** 这个探针默认 `--rounds 3` 就是为了这个。
