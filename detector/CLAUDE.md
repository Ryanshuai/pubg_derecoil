...
### ⚠ 它答不了「这一格是不是空的」，而且这条闸调不出来（2026-08-15）

**空的武器槽不画暗底板，它什么都不画** —— 裁下来的就是穿透过来的游戏世界，而草比图标的高频细节还多。867 帧带标签实测（`calibration/artifacts/ads/runs`，`meta.json` 的 `slot: 1`，1 号槽 kar98k、2 号槽全程空）：

| | Laplacian p1 / 中位 | 低于闸门 12 | `drawn()` 放行 | 拿到确信枪名 |
|---|---|---|---|---|
| 槽 1（有枪） | 380 / 1141 | 0.0% | 100% | — |
| **槽 2（空）** | 0.5 / **317** | **24.5%** | **75.5%** | **47%**（aug 347 / kar98k 40 / mg3 6 …） |

**1 号槽几乎总有枪、2 号槽经常空，所以幻影永远落在第二把上**——操作员报的「总是倾向于第二个枪」就是这个形状。

⚠ **换判据也不行，试过**：白+去饱和（`gun_tag` 用的那套）在这里同样分不开，过曝地面能在空槽里放 **1266** 个近白像素，而有枪的帧 p1 是 **0**。`gun_tag` 成立是因为字压在**暗框**上，HUD 图标没有框。

⚠ **而 `PLATE_INK_MIN` 当初是怎么定歪的，比这个数本身值钱**：它的两条证据里，第二条是「**无标签** HUD 帧里 12% 低于 10，而那些不可能是有枪的」——那句话只从**一侧**框住了空槽，读起来却像量过它。把同一批帧按 run 自己的 `slot` 字段标上（`meta.json` 一直都在），12% 就变成 24.5%，缺的那四分之三就是上面这张表。**一道用来拒绝某一类的闸，如果那一类从来没被标注过，就只能在它本来就放行的那一侧被测量。**（第三铁律推论四的又一个实例。）

### 那么谁有权说「空」：`gun_tag`，而且只在 Tab 里

```
枪架里那个「1」「2」   → gun_tag_detector → tab_watch → GameState.set_rack   ← 唯一作者
HUD 那两个裁图        → weapon_hud       → weapon_pred                      ← 只能命名，不能造枪
```

`GameState` 因此有三态而不是两态（`weapon_present`：`None` 没人看过 / `True` / `False`），**因为 `''` 同时是「没读出来」和「这里没有东西」，而 `or` 链把两者都解析成系统里最老的那个猜测**。play log 2026-08-15 13:06:50，相邻两行：`gun2 (no gun in the rack slot)` 和一张在 2 号槽印着 m416 的状态表——架子答对了，答案被扔掉了。

**而 HUD 只在第一次 Tab 读回之前作数**（操作员的规矩）：落地时还没有架子读回，一个凑合的名字好过没有；Tab 答过一次之后 HUD 只剩坏处——它看不见配件，所以说对了也加不出曲线，说错了却会改名 + 清 kit。**代价写在 `GameState.weapon_name` 里，是真的**：Tab 之后捡的枪要等下一次开 Tab 才改名。

⚠ **采集侧一个字没关**：`weapon_pred` 照写、`MISMATCH_TABLE` 照采，只是不再驱动——跟 `HighlightDetector` 2026-08-10 那次是同一个形状。闸在 `pixi run kit-persist`（两节，**四个变异全红**）和 `pixi run tab-watch`。
...
- `EXCLUDE` / `ONLY` / `SLOT_ONLY` 基本没实测。「某个槽只收部分配件」读不出来——收与不收留下的是同一个空 tile，只能逐个拖。⚠ **2026-08-10 有了第二条实测**：`('mp9','stock'): {'uzi_stock'}`，操作员给的，起因是夜跑两格 mp9 枪托全废。同一天 `GRIP_ONLY` 推广成 `SLOT_ONLY`（按 `(枪, 槽)` 索引）——因为同一个形状换到 stock 上出现了，而为一个事实并排放两张表正是它们漂开的方式。
