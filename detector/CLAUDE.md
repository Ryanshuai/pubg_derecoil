...
**三档的代价是量出来的，不是排的。** 9 个「整套和它的单件都测过」的格子（`pixi run python calibration/build_kit_factors.py` 会打这张表）：
...
**所以中间那档留着不是妥协，是 5 倍的差距。** 而它错在哪也是知道的：只错在耦合上（±15% 以内），wiki 那档还多错一层——**同一个件在不同枪上根本不是一个数**，`comp_smg` 在 mp5k 是 0.5907、在 vector 是 0.7197（5.5σ），wiki 对两把都说 0.85。
