...
`.row` 38 张 2026-08-09 删除，**没有替代**，所以现在没有任何东西按**图标**认库存行（`rows` 从 930/1050 掉到 ~80）。按**名字**认还在，而且更强：`data/templates/ocr_white/rows/`，白字模板，41 个件 + 36 个 `.nearby` 变体，见 `detector/row_name_detector.py`。删掉的 79 张 git 全跟踪，`git checkout data/templates/` 可回退。
