...
#
# `control/play_observer.py` 整条路是通的（`pixi run play-selftest`），关掉的理由
# 只有一个，而它不在这条路上:
#
# > 实战梭的每一个身份字段 —— 枪、配件、姿态、镜子 —— 都只有**检测器**一个来源。
# > 打架中途开不了 Tab，所以台上那道「同一个东西两个独立说法，对不上就拒绝」的
# > 闸门在这里没有第二个说法可比。
#
# 而检测层现在还读不准。于是每一梭都会诚实地落盘，说自己是别的东西 —— 不抛异常、
# 印出来的数也完全正常。这正是根 CLAUDE.md 第二条法则数过五次的那个形状，只是这
# 一次它会**按帧**发生，而且喂给一个会写回固件的拟合。
#
# **所以先修检测，再开这个。** 开关翻上来之前该成立的:
#   1. 枪 / 三个后坐力槽 / 姿态在实战画面上读得准，而且读不准时说得出来
#   2. 有东西能证明「这一梭真在镜里」（`ads_end` 全库 42/74 个文件仍是 False）
#   3. `MODEL.md` §4.1 那道外部检验在实战里还活着 —— 也就是补偿强度也逐梭轮转，
#      否则边玩边采默认是单臂的，而 `harness/verdict.py` 明写「一条臂 = 没验过」
PLAY_OBSERVE = False
