...
class RealHUDDataset(Dataset):
    """
    Real in-game screenshots from Manual/{weapon_class}/ folders.

    Filename: ocr_{gt}_dl_{model}_{h|l}_{hash}.png
              ocr_{gt}_dl_{model}_{h|l}_{conf}_{hash}.png
    """

    _HL_RE = re.compile(r'_([hl])_')

    def __init__(self, data_dir, layout, augment=True, oversample=1):
        self.layout = layout
        self.augment = augment
        self.input_h, self.input_w = layout.model_input_hw

        from dl_models.icon_layout import WEAPON_CLASSES
        cls_set = set(WEAPON_CLASSES)

        self.samples = []  # (path, gun_label, hl_label)
        for cls_name in os.listdir(data_dir):
            ...
            if cls_name == 'bg':
                ...
                gun_label = WEAPON_CLASSES.index(cls_name) + 1
            else:
                print(f'RealHUDDataset: skipping unknown class "{cls_name}"')
                continue

            for fname in os.listdir(cls_dir):
                if not fname.endswith('.png'):
                    continue
                m = self._HL_RE.search(fname)
                hl_label = {'h': 1, 'l': 2}.get(m.group(1), 0) if m else 0
                self.samples.append((os.path.join(cls_dir, fname), gun_label, hl_label))

        self.samples *= oversample
        print(f'RealHUDDataset: {len(self.samples)} samples '
    ...
    def __getitem__(self, idx):
        path, gun_label, hl_label = self.samples[idx]
        img = cv2.imread(path)
